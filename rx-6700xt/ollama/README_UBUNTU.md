# Ollama on Ubuntu with AMD RX 6700 XT (ROCm)

A guide for setting up Ollama in Docker on Ubuntu with an AMD RX 6700 XT GPU using ROCm. The RX 6700 XT is not officially supported by AMD's ROCm stack, but can be made to work with a GFX version override.
...
Ubuntu's default repositories ship an outdated version of Docker that does not include the Compose plugin. Install Docker from the official Docker repository instead.
...
Your Docker is from Ubuntu's default repos and is outdated. Follow Step 1 to install from Docker's official repo.
