# Ollama on Fedora with AMD RX 6700 XT (ROCm)

A guide for setting up Ollama in Docker on Fedora with an AMD RX 6700 XT GPU using ROCm. The RX 6700 XT is not officially supported by AMD's ROCm stack, but can be made to work with a GFX version override.
...
Fedora does not ship Docker in its default repositories. Install it from Docker's official repo. Note that Fedora uses `dnf` instead of `apt`.

```bash
# Remove any old or conflicting packages
...
Fedora ships with `firewalld` enabled by default. If you plan to access Ollama from another machine on your network, open port 11434:
...
Fedora uses SELinux by default which can sometimes interfere with Docker device passthrough. If you run into permission issues with `/dev/kfd` or `/dev/dri`, you may need to set the containers to run in permissive mode or apply the correct SELinux labels. The easiest workaround for a home server is:

```bash
sudo setenforce 0
```

To make this permanent, edit `/etc/selinux/config` and set `SELINUX=permissive`.

---

## 4. Set Up the Docker Named Volume
...
Make sure you installed `docker-compose-plugin` from Docker's official repo and not a standalone `docker-compose` binary.
...
Check that `HSA_OVERRIDE_GFX_VERSION=10.3.0` is set in your compose file and that `/dev/kfd` and `/dev/dri` are passed through correctly. Also check SELinux is not blocking device access (see Step 3).
...
On Fedora, make sure the Docker service is running:
```bash
